## Cool fixed camera with neck physics.
...
func _physics_process(_delta):
	...
	rotation = Vector3(defaultrot.x - (mometum_ro.x / 15.0), defaultrot.y - (mometum_ro.y / 15.0) -get_parent().final_steer * 0.25, defaultrot.z - (mometum_ro.z / 15.0))
